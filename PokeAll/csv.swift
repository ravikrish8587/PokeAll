...
public class CSV {
    ...
    func parseHeaders(fromLines lines: [String]) -> [String] {
        ...
        //lines[0].components(separatedBy: self.delimiter)
        return lines[0].characters.split(separator: ",").map(String.init)
    }
    ...
    func parseRows(fromLines lines: [String]) -> [Dictionary<String, String>] {
        ...
        for (lineNumber, line) in lines.enumerated() {
            ...
            let values = line.characters.split(separator: ",").map(String.init)//line.components(separatedBy: self.delimiter)
